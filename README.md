...
Приведённые ниже значения были получены путём усреднения по 8-ми сгенерированным картинкам
...
За пять часов на собственном макбуке получилось обучить диффузионную модель для генерации картинок котов размером 64×64. Обучение проводилось на 160 изображениях в 500 эпох. Далее на инференсе DDIM из-за меньшего числа шагов отработал сильно быстрее, но его результат несколько уступает по качеству DDPM. В целом, качество полученных изображений я считаю вполне приемлемым, на них явно вырисовываются коты, что с учётом не самых больших ресурсов на обучение, довольно неплохой результат.
